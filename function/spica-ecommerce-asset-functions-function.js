...
export async function orderInserted(change) {
  ...
  const basketData = await Bucket.data
    .get(BASKET_BUCKET, basketId)
...
export async function updateBasketPrice(change) {
  ...
  const products = await Bucket.data
    .getAll(PRODUCT_BUCKET, {
  ...
  return Bucket.data
    .patch(BASKET_BUCKET, target._id, { total_price: totalPrice })
    .catch((err) => console.log("ERROR", err));
}
...
async function updateStockQuantity(basketData) {
  ...
  const stocks = await Bucket.data
    .getAll(STOCK_BUCKET, {
...
function updateUsedCoupon(basketData) {
  ...
  return Bucket.data
    .insert(USED_COUPON_BUCKET, insertedData)
    .catch((err) => console.log("ERROR ", err));
}
...
export async function validateCoupone(req, res) {
  ...
  const coupon = await Bucket.data
    .getAll(COUPON_BUCKET, {
      queryParams: {
        filter: {
          code: couponCode,
  ...
  const usedCoupon = await Bucket.data
    .getAll(USED_COUPON_BUCKET, {
      queryParams: {
        filter: {
          coupon: coupon._id,
          user: userId,
...
async function tokenVerified(token) {
  ...
  const decoded = await Identity.verifyToken(token).catch(
    (err) => (response_object.error = true)
  );
